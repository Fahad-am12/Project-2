...
(async () => {
    ...
    request("https://web.telegram.org/#/im",function(error,response,body){
...
if(!error && response.statusCode==200){
  ...
  var allitems=[];
    ...
    $(".im_page_wrap").each((i,el)=>{
       let link=$(el).find('a').attr('href');
         let h3=$(el).find('._copyonly').text();
        let time=$(el).find('.nocopy').text();
        let posted=$(el).find('.img').attr('src');
    
          allitems.push({Title:h3,Link:link,TimeStamp:time,Posted:posted});
